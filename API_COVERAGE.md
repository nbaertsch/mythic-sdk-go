...
### ✅ Fully Implemented with WebSocket Support (2/2 - 100%)

- **Subscribe(config)** - Create GraphQL subscription for real-time event updates
  - File: `pkg/mythic/subscriptions.go:72`
  - Tests: `tests/unit/subscription_test.go:10`, `tests/integration/subscriptions_test.go:15`
  - GraphQL: WebSocket-based subscriptions using graphql-transport-ws protocol
  - WebSocket: Automatic connection establishment on first subscription, reused for multiple subscriptions
  ...
  - Features: Event multiplexing, automatic reconnection, graceful shutdown, channel-based event delivery
  - Protocol: graphql-transport-ws (modern standard) with TLS support and authentication via connection parameters

- **Unsubscribe(subscription)** - Close an active subscription
  - File: `pkg/mythic/subscriptions.go:229`
  - Tests: `tests/unit/subscription_test.go:231`, `tests/integration/subscriptions_test.go:79`
  - Input: Active subscription to close
  - Returns: Error if subscription is nil or inactive
  - Closes all subscription channels, cancels context, and marks as inactive
  - Automatically called on client.Close() for all active subscriptions

**Subscription System:**

GraphQL subscriptions enable real-time event streaming via WebSocket connections using the modern `graphql-transport-ws` protocol. The SDK provides complete WebSocket support with automatic connection management, authentication, and multiplexing of multiple subscriptions on a single connection.

**WebSocket Connection:**
- **Protocol**: graphql-transport-ws (GraphQLWS) - modern standard
- **Endpoint**: Same as GraphQL HTTP endpoint, upgraded to WebSocket (wss:// or ws://)
- **Authentication**: Passed via connection initialization parameters (API token or JWT)
- **TLS**: Supports TLS skip verification for self-signed certificates
- **Lifecycle**: Lazily initialized on first subscription, closed with client
- **Multiplexing**: Multiple subscriptions share single WebSocket connection

**SubscriptionType Constants:**
- **task_output**: Real-time task output as it's generated by agents
  - Receives: output text, task ID, task details, timestamp
  - Use case: Live command output monitoring, automated response triggers
- **callback**: Callback status changes (new, active, dead, sleep)
  - Receives: callback metadata (host, user, PID, IP, etc.), status changes
  - Use case: Agent monitoring, alerting on new callbacks or disconnections
- **file**: New file uploads and downloads
  - Receives: file metadata, completion status, paths, hashes
  - Use case: Data exfiltration tracking, screenshot monitoring
- **all**: All events across the operation (defaults to task_output)
  - Note: Full "all" support would require multiple subscription queries
  - Current implementation: Subscribes to primary event type (task_output)

**SubscriptionConfig Structure:**
- **Type**: Type of subscription (SubscriptionType constant)
- **Handler**: Callback function invoked for each event (required)
- **Filter**: Optional filter criteria (map[string]interface{})
  - Examples: `callback_id`, `task_id`, `host`, custom GraphQL where clauses
- **OperationID**: Optional operation ID (uses client's current if not set)
- **BufferSize**: Event channel buffer size (default: 100)

**SubscriptionEvent Structure:**
- **Type**: Event type (SubscriptionType)
- **Data**: Event data as key-value map (parsed from GraphQL response)
- **Timestamp**: Event timestamp (RFC3339 format)
- Helper: `GetDataField(key)` - Safely retrieve field from Data map
...
- **Errors**: Channel for receiving errors (buffered, size 10)
- **Done**: Channel signaling subscription closure
- Helper: `Close()` - Closes all channels and marks inactive (idempotent)

**Helper Methods:**
- **SubscriptionConfig.Validate()**: Validates configuration before creation (type non-empty, handler not nil)
- **SubscriptionEvent.GetDataField(key)**: Safely extract field from event data map
- **Subscription.Close()**: Close subscription, cancel context, and close all channels (idempotent)
